# TempMailApiByBoomlify Ruby SDK



The Ruby SDK for the TempMailApiByBoomlify API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Domain` — with named operations (`load`/`create`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```ruby
client = TempMailApiByBoomlifySDK.test(testopts, sdkopts)
```
...
raise a `TempMailApiByBoomlifyError` (a `StandardError` subclass), so wrap
...
inbox = client.Inbox.load({ "id" => "inbox_id", "token" => "token" })
```
...
The main module (`TempMailApiByBoomlify_sdk`) exports the SDK class
