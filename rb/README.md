...
The Ruby SDK for the TempMailApiByBoomlify API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
result, err = client.Domain(nil).load({ "id" => "example_id" }, nil)
raise err if err
puts result
```
...
result, err = client.TempMailApiByBoomlify(nil).load(
  { "id" => "test01" }, nil
)
