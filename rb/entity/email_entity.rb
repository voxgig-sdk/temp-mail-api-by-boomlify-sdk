# TempMailApiByBoomlify SDK Email entity
...
class EmailEntity
  ...
  def data_set(args)
    if args
      @_data = TempMailApiByBoomlifyHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = TempMailApiByBoomlifyHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # Create a new Email.
  #
  # @param reqdata [EmailCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [Email, Hash] the created Email; raises TempMailApiByBoomlifyError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        if ctx.result.resdata
          @_data = TempMailApiByBoomlifyHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
