...
module TempMailApiByBoomlifyUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = TempMailApiByBoomlifyResponse.new({})
    ...
    if fetch_err
      ...
      response = TempMailApiByBoomlifyResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = TempMailApiByBoomlifyResponse.new(fetched)
    else
