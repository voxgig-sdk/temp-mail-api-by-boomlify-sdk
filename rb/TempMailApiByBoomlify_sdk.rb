...
require_relative 'TempMailApiByBoomlify_types'
...
class TempMailApiByBoomlifySDK
  ...
  def initialize(options = {})
    ...
    feature_opts = TempMailApiByBoomlifyHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = TempMailApiByBoomlifyHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, TempMailApiByBoomlifyFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    TempMailApiByBoomlifyUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = TempMailApiByBoomlifyHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = TempMailApiByBoomlifyHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = TempMailApiByBoomlifyHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => TempMailApiByBoomlifyError.new(
        "#{op}_allow",
        "TempMailApiByBoomlifySDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue TempMailApiByBoomlifyError => err
    ...
    if fetched.is_a?(Hash)
      status = TempMailApiByBoomlifyHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = TempMailApiByBoomlifyError.new(
        "graphql_error", "TempMailApiByBoomlifySDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = TempMailApiByBoomlifySDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
