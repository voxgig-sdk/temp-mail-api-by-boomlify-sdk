...
// TempMailApiByBoomlify SDK utility: result_basic

class TempMailApiByBoomlifyResultBasic
{
    public static function call(TempMailApiByBoomlifyContext $ctx): ?TempMailApiByBoomlifyResult
    {
        ...
        if ($result && $response) {
            ...
            if ($result->status >= 400) {
                ...
                if ($result->err) {
                    $prev = ($result->err instanceof TempMailApiByBoomlifyError) ? $result->err->msg : (string)$result->err;
