...
class DomainEntity
{
    ...
    {
        if ($args) {
            $this->_data = TempMailApiByBoomlifyHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = TempMailApiByBoomlifyHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *   SDK boundary; throws TempMailApiByBoomlifyError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = TempMailApiByBoomlifyHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
