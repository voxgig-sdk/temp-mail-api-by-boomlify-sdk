...
// TempMailApiByBoomlify SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = TempMailApiByBoomlifyOmni::makeStructRunner(
    ...
    {
        return TempMailApiByBoomlifyOmni::objmodel(TempMailApiByBoomlifyOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return TempMailApiByBoomlifyOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return TempMailApiByBoomlifyOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(TempMailApiByBoomlifyOmni::structfix($now, Struct::undef()))
                        !== json_encode(TempMailApiByBoomlifyOmni::structfix($val, Struct::undef()))) {
