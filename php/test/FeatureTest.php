...
// TempMailApiByBoomlify SDK feature test
//
...
class FtCtrl extends TempMailApiByBoomlifyControl
{
...
class FtClient
{
    ...
    private TempMailApiByBoomlifyUtility $utility;

    public function __construct(TempMailApiByBoomlifyUtility $utility, array $options)
    {
    ...
    public function get_utility(): TempMailApiByBoomlifyUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (TempMailApiByBoomlifyContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public TempMailApiByBoomlifyContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (TempMailApiByBoomlifyContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = TempMailApiByBoomlifyConfig::make_feature($name);
    ...
    private static function buildUrl(TempMailApiByBoomlifySpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof TempMailApiByBoomlifyError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new TempMailApiByBoomlifyResponse($fetched);
        }
    ...
    private function fail(TempMailApiByBoomlifyContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (TempMailApiByBoomlifyContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
