...
// TempMailApiByBoomlify SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class TempMailApiByBoomlifyTestFeature extends TempMailApiByBoomlifyBaseFeature
{
    ...
    public function init(TempMailApiByBoomlifyContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (TempMailApiByBoomlifyContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = TempMailApiByBoomlifyParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (TempMailApiByBoomlifyContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(TempMailApiByBoomlifyContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = TempMailApiByBoomlifyParam::call($ctx, $k);
