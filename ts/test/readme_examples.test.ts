...
//     (\`new TempMailApiByBoomlifySDK(...)\` / \`TempMailApiByBoomlifySDK.test(...)\` become
//     \`TempMailApiByBoomlifySDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'TempMailApiByBoomlifySDK'
...
// rewrite real client construction (\`new TempMailApiByBoomlifySDK(...)\` and any
// \`TempMailApiByBoomlifySDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+TempMailApiByBoomlifySDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/TempMailApiByBoomlifySDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
