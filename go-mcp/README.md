# temp-mail-api-by-boomlify-mcp

[MCP](https://modelcontextprotocol.io) server exposing the TempMailApiByBoomlify SDK as
two agent tools — `temp-mail-api-by-boomlify_list` and `temp-mail-api-by-boomlify_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/temp-mail-api-by-boomlify-mcp)
make build

# 2. Provide credentials via the environment
export TEMP_MAIL_API_BY_BOOMLIFY_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user temp-mail-api-by-boomlify \
  -- /absolute/path/to/temp-mail-api-by-boomlify-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./temp-mail-api-by-boomlify-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// temp-mail-api-by-boomlify_list: first page of records
{ "entity": "domain" }
{ "entity": "domain", "query": { } }

// temp-mail-api-by-boomlify_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/temp-mail-api-by-boomlify-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user temp-mail-api-by-boomlify \
     -- "$PWD"/dist/*/temp-mail-api-by-boomlify-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `temp-mail-api-by-boomlify_list` and `temp-mail-api-by-boomlify_load` tools now appear
   in new sessions. Ask the agent to *"list domain using temp-mail-api-by-boomlify"*
   and it calls `temp-mail-api-by-boomlify_list` with `{"entity":"domain"}`.
...
```sh
export TEMP_MAIL_API_BY_BOOMLIFY_APIKEY=sk_live_xxx            # API key
export TEMP_MAIL_API_BY_BOOMLIFY_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `temp-mail-api-by-boomlify_list` tool
...
### Call the `temp-mail-api-by-boomlify_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `temp-mail-api-by-boomlify_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `temp-mail-api-by-boomlify_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `TEMP_MAIL_API_BY_BOOMLIFY_APIKEY` | API key sent with every request. |
| `TEMP_MAIL_API_BY_BOOMLIFY_BASE` | Optional override of the API base URL. |
...
```sh
./temp-mail-api-by-boomlify-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"temp-mail-api-by-boomlify_load","arguments":{"entity":"domain","query":{"id":1}}}}'
```
