...
def make_error_util(ctx, err):
    if ctx is None:
        from projectname_sdk.core.context import TempMailApiByBoomlifyContext
        ctx = TempMailApiByBoomlifyContext({}, None)
    ...
    result = ctx.result
    if result is None:
        result = TempMailApiByBoomlifyResult({})
    result.ok = False
    ...
    errmsg = ""
    if isinstance(err, TempMailApiByBoomlifyError):
        errmsg = err.msg
    ...
    sdk_err = TempMailApiByBoomlifyError("", msg, ctx)
