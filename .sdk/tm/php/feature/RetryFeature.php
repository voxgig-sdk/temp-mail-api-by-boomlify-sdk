...
// TempMailApiByBoomlify SDK retry feature
...
class TempMailApiByBoomlifyRetryFeature extends TempMailApiByBoomlifyBaseFeature
{
    ...
    public function init(TempMailApiByBoomlifyContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (TempMailApiByBoomlifyContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(TempMailApiByBoomlifyContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(TempMailApiByBoomlifyContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
