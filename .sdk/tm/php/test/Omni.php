...
//    only - a typed TempMailApiByBoomlifyContext there would make every ctx
...
final class TempMailApiByBoomlifyOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new TempMailApiByBoomlifyContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new TempMailApiByBoomlifySpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new TempMailApiByBoomlifyResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new TempMailApiByBoomlifyError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new TempMailApiByBoomlifyResponse($clean['response']);
    ...
    public static function err_from_map($m): ?TempMailApiByBoomlifyError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new TempMailApiByBoomlifyError(is_string($code) ? $code : '', $msg);
    }
