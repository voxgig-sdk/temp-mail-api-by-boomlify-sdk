...
module TempMailApiByBoomlifyOmni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if TempMailApiByBoomlifyOmni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = TempMailApiByBoomlifyOmni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| TempMailApiByBoomlifyOmni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || TempMailApiByBoomlifyOmni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = TempMailApiByBoomlifySpec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = TempMailApiByBoomlifyError.new('', err_map['message'])
      end
    end
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { TempMailApiByBoomlifyOmni.errify(err) }
