# TempMailApiByBoomlify SDK test runner
...
class TempMailApiByBoomlifyTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            TempMailApiByBoomlifyTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = TempMailApiByBoomlifyTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = TempMailApiByBoomlifyTestRunner.getenv("TEMP_MAIL_API_BY_BOOMLIFY_TEST_LIVE")
        override = TempMailApiByBoomlifyTestRunner.getenv("TEMP_MAIL_API_BY_BOOMLIFY_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = TempMailApiByBoomlifyTestRunner.getenv(key)
        ...
        explain = TempMailApiByBoomlifyTestRunner.getenv("TEMP_MAIL_API_BY_BOOMLIFY_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if TempMailApiByBoomlifyTestRunner._test_control is not None:
            return TempMailApiByBoomlifyTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                TempMailApiByBoomlifyTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            TempMailApiByBoomlifyTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = TempMailApiByBoomlifyTestRunner.load_test_control()
...
def load_env_local():
    TempMailApiByBoomlifyTestRunner.load_env_local()


def env_override(m):
    return TempMailApiByBoomlifyTestRunner.env_override(m)


def entity_data(v):
    return TempMailApiByBoomlifyTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return TempMailApiByBoomlifyTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return TempMailApiByBoomlifyTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return TempMailApiByBoomlifyTestRunner.load_test_control()


def live_client_options():
    return TempMailApiByBoomlifyTestRunner.live_client_options()


def live_delay_ms():
    return TempMailApiByBoomlifyTestRunner.live_delay_ms()
