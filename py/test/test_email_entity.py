    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set TEMP_MAIL_API_BY_BOOMLIFY_TEST_EMAIL_ENTID JSON to run live")
        client = setup["client"]
...
def _email_basic_setup(extra):
    ...
    _entid_env_raw = os.environ.get(
        "TEMP_MAIL_API_BY_BOOMLIFY_TEST_EMAIL_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("TEMP_MAIL_API_BY_BOOMLIFY_TEST_EMAIL_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("TEMP_MAIL_API_BY_BOOMLIFY_TEST_LIVE") == "TRUE":
        ...
        client = TempMailApiByBoomlifySDK(helpers.to_map(merged_opts))

    _live = env.get("TEMP_MAIL_API_BY_BOOMLIFY_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("TEMP_MAIL_API_BY_BOOMLIFY_TEST_EXPLAIN") == "TRUE",
        "live": _live,
