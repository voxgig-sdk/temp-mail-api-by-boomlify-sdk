...
from tempmailapibyboomlify_sdk.utility import register

# Load features
from tempmailapibyboomlify_sdk.feature.base_feature import TempMailApiByBoomlifyBaseFeature
from tempmailapibyboomlify_sdk.features import _has_feature, _make_feature
...
class TempMailApiByBoomlifySDK:
    ...
    def __init__(self, options=None):
        ...
        utility = TempMailApiByBoomlifyUtility()
        self._utility = utility

        from tempmailapibyboomlify_sdk.config import shared_config
    ...
    def get_utility(self):
        return TempMailApiByBoomlifyUtility.copy(self._utility)
    ...
    def graphql(self, query, variables=None, ctrl=None):
        ...
        if isinstance(errors, list) and 0 < len(errors):
            ...
            res["ok"] = False
            res["err"] = Exception("TempMailApiByBoomlifySDK: graphql: " + str(msg))
            res["graphql"] = errors
    ...
    def Domain(self, data=None) -> "DomainEntity":
        ...
        from tempmailapibyboomlify_sdk.entity.domain_entity import DomainEntity
        return DomainEntity(self, data)


    def Email(self, data=None) -> "EmailEntity":
        """Entity factory: client.Email().list() / client.Email().load({"id": ...})."""
        from tempmailapibyboomlify_sdk.entity.email_entity import EmailEntity
        return EmailEntity(self, data)


    def Inbox(self, data=None) -> "InboxEntity":
        """Entity factory: client.Inbox().list() / client.Inbox().load({"id": ...})."""
        from tempmailapibyboomlify_sdk.entity.inbox_entity import InboxEntity
        return InboxEntity(self, data)



    @classmethod
    def test(cls, testopts=None, sdkopts=None) -> "TempMailApiByBoomlifySDK":
        if sdkopts is None:
            sdkopts = {}
