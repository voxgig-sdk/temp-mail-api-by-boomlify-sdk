...
from tempmailapibyboomlify_sdk.core.helpers import get_ctx_prop, to_map
...
class TempMailApiByBoomlifyContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = TempMailApiByBoomlifyControl()
        ...
        if isinstance(sp, TempMailApiByBoomlifySpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, TempMailApiByBoomlifyResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, TempMailApiByBoomlifyResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return TempMailApiByBoomlifyOperation({})
        ...
        op = TempMailApiByBoomlifyOperation({
            "entity": entname,
            "name": opname,
            "input": inpt,
            "points": points,
    ...
    def make_error(self, code, msg):
        return TempMailApiByBoomlifyError(code, msg, self)
