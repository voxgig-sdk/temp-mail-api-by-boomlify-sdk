# TempMailApiByBoomlify Python SDK

The Python SDK for the TempMailApiByBoomlify API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Domain(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.TempMailApiByBoomlify(None).load(
    {"id": "test01"}, None
)
