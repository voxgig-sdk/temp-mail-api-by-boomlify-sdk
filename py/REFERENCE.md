# TempMailApiByBoomlify Python SDK Reference

Complete API reference for the TempMailApiByBoomlify Python SDK.
...
#### `TempMailApiByBoomlifySDK.test(testopts=None, sdkopts=None)`
...
```python
result = client.Inbox().load({"id": "inbox_id", "token": "token"})
```
