...
The Lua SDK for the TempMailApiByBoomlify API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Domain(nil):load({ id = "example_id" }, nil)
...
local result, err = client:TempMailApiByBoomlify(nil):load(
  { id = "test01" }, nil
)
