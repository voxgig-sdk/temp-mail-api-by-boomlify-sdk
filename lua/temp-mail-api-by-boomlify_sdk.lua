...
require("temp-mail-api-by-boomlify_types")
...
local TempMailApiByBoomlifySDK = {}
TempMailApiByBoomlifySDK.__index = TempMailApiByBoomlifySDK
...
TempMailApiByBoomlifySDK._make_feature = _make_feature


function TempMailApiByBoomlifySDK.new(options)
  local self = setmetatable({}, TempMailApiByBoomlifySDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function TempMailApiByBoomlifySDK:options_map()
...
function TempMailApiByBoomlifySDK:get_utility()
  return Utility.copy(self._utility)
end


function TempMailApiByBoomlifySDK:get_root_ctx()
  return self._rootctx
end


function TempMailApiByBoomlifySDK:prepare(fetchargs)
...
function TempMailApiByBoomlifySDK:direct(fetchargs)
...
function TempMailApiByBoomlifySDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function TempMailApiByBoomlifySDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "TempMailApiByBoomlifySDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function TempMailApiByBoomlifySDK:_raw_request(fetchargs)
  local utility = self._utility
...
function TempMailApiByBoomlifySDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "TempMailApiByBoomlifySDK: graphql: " .. msg
    res.graphql = errors
  end
...
function TempMailApiByBoomlifySDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = TempMailApiByBoomlifySDK.new(sdkopts)
  sdk.mode = "test"
...
return TempMailApiByBoomlifySDK
