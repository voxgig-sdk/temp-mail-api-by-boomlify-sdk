# TempMailApiByBoomlify Lua SDK Reference

Complete API reference for the TempMailApiByBoomlify Lua SDK.
...
```lua
local result, err = client:Inbox():load({ id = "inbox_id", token = "token" })
```
