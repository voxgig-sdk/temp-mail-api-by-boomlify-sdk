...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("TEMPMAILAPIBYBOOMLIFY_TEST_LIVE") == "TRUE" ||
		os.Getenv("TEMPMAILAPIBYBOOMLIFY_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("TEMPMAILAPIBYBOOMLIFY_TEST_EXPLAIN"); explain != "" {
