...
import (
	...
	omni "github.com/voxgig-sdk/temp-mail-api-by-boomlify-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.TempMailApiByBoomlifyError); is {
				out := map[string]any{
					"name":    "TempMailApiByBoomlifyError",
...
func omniCtx(arg any, client *sdk.TempMailApiByBoomlifySDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.TempMailApiByBoomlifySDK); is {
			client = live
