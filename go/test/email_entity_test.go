...
func TestEmailEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set TEMP_MAIL_API_BY_BOOMLIFY_TEST_EMAIL_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func emailBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("TEMP_MAIL_API_BY_BOOMLIFY_TEST_EMAIL_ENTID")
	...
	idmapResolved := core.ToMapAny(env["TEMP_MAIL_API_BY_BOOMLIFY_TEST_EMAIL_ENTID"])
