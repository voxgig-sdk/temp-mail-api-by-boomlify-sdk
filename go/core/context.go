...
func NewContext(ctxmap map[string]any, basectx *Context) *Context {
	...
	if c := getCtxProp(ctxmap, "client"); c != nil {
		if sdk, ok := c.(*TempMailApiByBoomlifySDK); ok {
			ctx.Client = sdk
		}
	}
...
func (ctx *Context) MakeError(code string, msg string) *TempMailApiByBoomlifyError {
	return NewTempMailApiByBoomlifyError(code, msg, ctx)
}
