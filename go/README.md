# TempMailApiByBoomlify Golang SDK



The Golang SDK for the TempMailApiByBoomlify API — an entity-oriented client using standard Go conventions. No generics required; data flows as `map[string]any`.

It exposes the API as capitalised, semantic **Entities** — e.g. `client.Domain(nil)` — each with the same small set of operations (`Load`, `Create`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
## Install
```bash
go get github.com/voxgig-sdk/temp-mail-api-by-boomlify-sdk/go@latest
```

The Go module proxy resolves the version from the `go/vX.Y.Z` GitHub
release tag — see [Releases](https://github.com/voxgig-sdk/temp-mail-api-by-boomlify-sdk/releases) for the available versions.
...
```bash
go mod edit -replace github.com/voxgig-sdk/temp-mail-api-by-boomlify-sdk/go=../temp-mail-api-by-boomlify-sdk/go
```
...
func main() {
    ...
    domain, err := client.Domain(nil).Load(nil, nil)
    if err != nil {
        panic(err)
    }
    fmt.Println(domain)
...
### NewTempMailApiByBoomlifySDK

```go
func NewTempMailApiByBoomlifySDK(options map[string]any) *TempMailApiByBoomlifySDK
```
...
```go
func TestSDK(testopts map[string]any, sdkopts map[string]any) *TempMailApiByBoomlifySDK
```

Creates a test-mode client with mock transport. Both arguments may be `nil`.

### TempMailApiByBoomlifySDK methods
...
| `Domain` | `(data map[string]any) TempMailApiByBoomlifyEntity` | Create a Domain entity instance. |
| `Email` | `(data map[string]any) TempMailApiByBoomlifyEntity` | Create an Email entity instance. |
| `Inbox` | `(data map[string]any) TempMailApiByBoomlifyEntity` | Create an Inbox entity instance. |

### Entity interface (TempMailApiByBoomlifyEntity)

All entities implement the `TempMailApiByBoomlifyEntity` interface.
...
```go
inbox, err := client.Inbox(nil).Load(map[string]any{"id": "inbox_id", "token": "token"}, nil)
if err != nil {
    panic(err)
}
