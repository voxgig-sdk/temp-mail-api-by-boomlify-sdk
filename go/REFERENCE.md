# TempMailApiByBoomlify Golang SDK Reference

Complete API reference for the TempMailApiByBoomlify Golang SDK.
...
#### `TestSDK(testopts, sdkopts map[string]any) *TempMailApiByBoomlifySDK`
...
#### `Domain(data map[string]any) TempMailApiByBoomlifyEntity`

Create a new `Domain` entity instance. Pass `nil` for no initial data.

#### `Email(data map[string]any) TempMailApiByBoomlifyEntity`

Create a new `Email` entity instance. Pass `nil` for no initial data.

#### `Inbox(data map[string]any) TempMailApiByBoomlifyEntity`
...
```go
result, err := client.Inbox(nil).Load(map[string]any{"id": "inbox_id", "token": "token"}, nil)
if err != nil {
    panic(err)
}
fmt.Println(result)
