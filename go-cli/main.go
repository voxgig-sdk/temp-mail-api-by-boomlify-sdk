// Command TempMailApiByBoomlify-cli is a CLI + REPL for the TempMailApiByBoomlify SDK,
...
const prompt = "temp-mail-api-by-boomlify"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      TEMP_MAIL_API_BY_BOOMLIFY_APIKEY (api key), TEMP_MAIL_API_BY_BOOMLIFY_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: TEMP_MAIL_API_BY_BOOMLIFY_APIKEY carries the API key and
	// TEMP_MAIL_API_BY_BOOMLIFY_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("TEMP_MAIL_API_BY_BOOMLIFY_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("TEMP_MAIL_API_BY_BOOMLIFY_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewTempMailApiByBoomlifySDK(opts)
