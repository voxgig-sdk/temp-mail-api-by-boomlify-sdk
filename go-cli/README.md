# temp-mail-api-by-boomlify-cli

boru-driven command-line client **and** interactive REPL for the TempMailApiByBoomlify
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/temp-mail-api-by-boomlify-cli)
make build

# 2. See usage (words, entities, env vars)
./temp-mail-api-by-boomlify-cli --help

# 3. Provide credentials once, via the environment
export TEMP_MAIL_API_BY_BOOMLIFY_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./temp-mail-api-by-boomlify-cli load 1 domain            # {id:1} shorthand
./temp-mail-api-by-boomlify-cli load '{id:1}' domain       # explicit match map

# 5. Override the API base URL for a single call
TEMP_MAIL_API_BY_BOOMLIFY_BASE=https://api.example.com ./temp-mail-api-by-boomlify-cli load 1 domain

# 6. No arguments -> interactive REPL
./temp-mail-api-by-boomlify-cli
temp-mail-api-by-boomlify> load 1 domain
temp-mail-api-by-boomlify> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/temp-mail-api-by-boomlify-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/temp-mail-api-by-boomlify-cli load 1 domain
   ```
...
```sh
./temp-mail-api-by-boomlify-cli load 1 domain          # scalar shorthand for {id:1}
./temp-mail-api-by-boomlify-cli load '{id:1}' domain     # explicit match map
```
...
./temp-mail-api-by-boomlify-cli load 1 domain
```
...
Run with no arguments to open a REPL (prompt `temp-mail-api-by-boomlify>`). Each line is
evaluated as its own boru expression:

```text
$ ./temp-mail-api-by-boomlify-cli
temp-mail-api-by-boomlify> load 1 domain
temp-mail-api-by-boomlify> /help
temp-mail-api-by-boomlify> /quit
```
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/temp-mail-api-by-boomlify-cli`. |
...
(`./temp-mail-api-by-boomlify-cli <expr>`) and interactively (the REPL), and expressions compose the
